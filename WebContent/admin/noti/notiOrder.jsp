...
<div class="wrap">
	...
	<form action="notiOrderProc.jsp">
		...
		<!-- 순서 조정 버튼 -->
		<div>
		<span style="font-size: 10pt;">선택한 게시물 </span>
		<input type="button" value="맨 위로" onclick="notiDel(this.form)">
		<input type="button" value="위로" onclick="notiDel(this.form)">
		<input type="button" value="아래로" onclick="notiDel(this.form)">
		<input type="button" value="맨 아래로" onclick="notiDel(this.form)">
		</div>
		...
		<table class="list">
			...
			<!-- 페이지 리스트 -->
			<%
			String paging=new Paging().paging(totalRecord,nowPage,recordPerPage,col,word,"notiOrder.jsp");
			out.print(paging);
			 %>
			</td></tr>
			<%
	...
	<!-- 검색 시작 -->
	<form method="get" action="notiOrder.jsp" onsubmit="return searchCheck(this)" class="search" style="display:inline;'">
	<select name="col">
