...
<form name="modForm" method="post" action="memberModifyProc.jsp" onsubmit="return memberCheck(this)">
	...
	<table border="1" class="writefrm">
		...
		<tr>
			<th>*이름</th>
			<td><input type="text" name="mname" id="mname" value="<%=dto.getMname()%>" size="10" required></td>
			<td><h6 style="margin:0 auto;">2~20자 이내의 이름</h6></td>
		</tr>
		<tr>
			<th>*이메일</th>
			<td colspan="2"><input type="text" name="email" id="email" value="<%=dto.getEmail()%>" size="30" readonly> <input type="button" name="emailbt" value="Email 중복확인" onclick="emailCheck()">
			</td>
		</tr>
		<tr>
			<th>전화번호</th>
			<td colspan="2"><input type="text" name="tel" id="tel" value="<%=Utility.checkNull(dto.getTel())%>" size="10"></td>
		</tr>
		<tr>
			<th>우편번호</th>
			<td colspan="2"><input type="text" name="zipcode" id="zipcode" value="<%=Utility.checkNull(dto.getZipcode())%>" size="7" readonly> <input type="button" value="주소찾기" onclick="DaumPostcode()">
			</td>
		</tr>
		<tr>
			<th>주소</th>
			<td colspan="2"><input type="text" name="address1" id="address1" value="<%=Utility.checkNull(dto.getAddress1())%>" size="45" readonly></td>
		</tr>
		<tr>
			<th>나머지주소</th>
			<td colspan="2"><input type="text" name="address2" id="address2" value="<%=Utility.checkNull(dto.getAddress2())%>" size="45"></td>
		</tr>
		<tr>
			<th>*직업</th>
			<td colspan="2"><select name="job" id="job">
					...
					<option value="A02" <%if(dto.getJob().equals("A02")) out.print("selected");%>>전산관련직</option>
